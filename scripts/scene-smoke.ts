...
const s = new Scene([]);
const r = s.apply([
  { op: "add_frame", ref: "hld", name: "High-level design", place: { at: { x: 0, y: 0 } }, width: 1200, height: 600 },
  ...
  { op: "add_node", label: "Postgres", shape: "ellipse", place: { below: "api" }, color: "green" },
  ...
  { op: "update", target: "api", label: "API Service (stateless)", move: { right_of: "Load Balancer", gap: 200 } },
], "agent");
...
console.log("after remove edges:", g.edges.length, "nodes:", g.nodes.map((n: any) => n.label));
console.log("changed", s.changedElements().length, "indices monotonic", s.live().every((e, i, a) => i === 0 || a[i - 1].index! < e.index!));
