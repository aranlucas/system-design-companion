...
- **`tidy`** (MCP tool, **Tidy** button, and automatically on the frames each agent patch touched): attach loose items to the frame they sit in, bind loose arrow ends that `graph()` already reads as an edge (both ends on nodes, nearest node wins), wrap over-long notes, snap nearly-aligned boxes onto the largest box in the row or column, even out roughly even gaps in a row or column, separate overlaps with the smallest push (a user group moves as one piece; when pushing sideways or down cost about the same, the push that crosses fewer arrows wins), fit frames (a frame tidy grew shrinks back as its contents allow, never below the size someone chose, kept in `customData.autoFit`), pull overlapping frames apart, and route connections inside their frame. It never changes connections, labels, colours or relative order. It is idempotent: a second run changes nothing. The button tidies only the frames touched by the selection, if any. A snapshot is taken first, only when something will change, and the full dagre `layout` stays opt-in. `tests/scene-tidy.test.ts` checks, with the metrics `scripts/tidy-check.ts` prints, that tidy makes no template or seeded messy diagram worse.
